...
//			regressionTest = true;
	...
	// Proze�-Instanz ein entsprechender Offset hinzu addiert.
	...
	if (tdfProcessGroup.getParameter("backup_objectid", backupOwnOid) == isNotOk)
        ...
        else
        {
          SesConfig sesConfig(SesConfig::NONE);
			...
			// Aktivieren der des Dispatchers zur Message-�bermittlung
            tdfAccess->enableApplicationMessageBox();
            idaTrackData(("enableApplicationMessageBox() ...  done"));

			// Wir merken uns die Instanz in einer Liste, damit sie sp�ter
